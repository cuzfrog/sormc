...
object Instance {
  ...
  trait Api extends Logging {
    ...
    = {
      ...
      = mappings.getOrElse(r, throw new SormException("Entity `" + r.name + "` is not registered"))
    ...
    = connector.withConnection { cx =>
      jdbc.Statement.simple(template, values)
        ...
        .ensuring(
          _.headOption.forall(k => k.keys == Set("id")),
    ...
    lazy val now = {
      ...
      () => base.plus(System.currentTimeMillis() - systemBase, ChronoField.MILLI_OF_DAY.getBaseUnit)
    }
  ...
  (entities: Traversable[Entity],
   ...
   timeout: Int)
    ...
    = {
      ...
      settings.keys
        .zipBy {
          new EntityMapping(_, None, settings)
        }
        .toMap
    }
